...
- **Workflow paths updated** to remove `FX/` prefix from all directory references, now that `app/` and `backend/` are in the repository root.
...
- Paths have been updated to reference `app/` and `backend/` directly in the root, not under `FX/`.
