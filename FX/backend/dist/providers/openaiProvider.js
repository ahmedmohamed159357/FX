...
exports.streamCreativeText = streamCreativeText;
exports.generateCreativeVariations = generateCreativeVariations;
exports.getOpenAISetupGuide = getOpenAISetupGuide;
 ...
 */
async function streamCreativeText(prompt, systemPrompt, config, onChunk) {
    try {
...
async function generateCreativeVariations(prompt, systemPrompt, config, count = 3) {
    ...
    for (let i = 0; i < count; i++) {
        const variation = await generateCreativeText(`${prompt}\n\nGenerate variation ${i + 1} of ${count}. Make it distinct and unique.`, systemPrompt, config);
 ...
 */
function generateMockOpenAIResponse(prompt, systemPrompt) {
