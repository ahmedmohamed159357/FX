...
// ─────────────────────────────────────────────
// TECHNIQUES  (unchanged from v3 — same fallbacks)
// ─────────────────────────────────────────────
...
export async function synthesizeLateralThinking(
  ...
  const creativeBreakthrough = ctx.language === 'ar'
    ? '🎯 نحتضن التناقض: اجعل النسيان مستحيلاً بجعل التذكر لا يُقاوم.'
    : '🎯 Embrace the paradox: make forgetting impossible by making remembering irresistible.'

  const raw = { provocation, analogies, randomStimulus, oppositeThinking, synthesizedInsight, creativeBreakthrough }
...
// ─────────────────────────────────────────────
// CONSTRAINT REVERSALS
// ─────────────────────────────────────────────

export const constraintReversals: Record<string, string> = {
  'people forget':           'What if forgetting is the challenge to celebrate?',
  'reminder is annoying':    'What if the reminder is an invitation to joy?',
  'hydration is boring':     'What if hydration is the most intimate self-love act?',
  'habit formation is hard': 'What if the habit already exists — we just sync with it?',
  'النسيان':                 'ماذا لو كان النسيان هو التحدي الذي نحتفل به؟',
  'التذكير مزعج':            'ماذا لو كان التذكير دعوة للبهجة؟',
  'تكوين العادة صعب':        'ماذا لو كانت العادة موجودة بالفعل؟',
}
